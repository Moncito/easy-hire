...
export default function DashboardJobPerformance({ rows }: Props) {
  const topApplicants = Math.max(...rows.map((row) => row.applicants), 0);

  return (
    <DashboardSurface>
      <div className="mb-4">
        <p className="text-[10px] font-bold uppercase tracking-wider text-navy/60">Listing analytics</p>
        <h2 className="font-display text-lg font-bold tracking-tight text-ink">Job performance</h2>
        <p className="mt-1 text-xs text-ink/45">Compare views and applicant conversion across active roles.</p>
      </div>

      <div className="overflow-x-auto">
        <table className="w-full min-w-[480px] text-left text-sm">
          <thead>
            <tr className="border-b border-ink/[0.06] text-[10px] font-bold uppercase tracking-wider text-ink/40">
                ...
                <tr key={row.id} className="border-b border-ink/[0.04] last:border-0">
                  <td className="py-3 pr-4">
                    <Link
                      ...
                      title={row.title}
                    >
                      <span className="line-clamp-1 font-semibold text-ink transition group-hover:text-teal">
                        {splitTitle(row.title)}
                      </span>
                      <span className="mt-1 block h-1 max-w-[120px] overflow-hidden rounded-full bg-ink/5">
                        <span
                          className="block h-full rounded-full bg-teal/70 transition-all"
                    ...
                    {row.conversion === null ? (
                      ...
                      <span
                        className={`font-data text-sm font-bold ${
                          row.conversion >= 20 ? "text-teal" : "text-ink/70"
                        }`}
                      >
                        {row.conversion}%
                      </span>
