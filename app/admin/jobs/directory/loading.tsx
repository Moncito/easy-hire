/**
 * Skeleton matching the final `/admin/jobs/directory` layout: filter bar +
 * a dense table shell with header + row placeholders.
 */
export default function AdminJobsDirectoryLoading() {
  return (
    <div className="mx-auto max-w-6xl animate-pulse">
      ...
      <div className="mb-4 flex items-center justify-between gap-3">
        <div className="h-9 w-48 rounded-xl bg-ink/10" />
        <div className="h-9 w-72 rounded-xl bg-ink/10" />
      </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-ink/5 bg-white">
        ...
        {Array.from({ length: 10 }).map((_, i) => (
          <div key={i} className="flex h-8 items-center gap-3 border-b border-ink/5 px-3">
            ...
            <div className="h-3 w-16 rounded bg-ink/5" />
            <div className="ml-auto h-3 w-20 rounded bg-ink/10" />
