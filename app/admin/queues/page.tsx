    ...
    <div className="mx-auto max-w-6xl">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {KIND_ORDER.map((kind) => {
          ...
          const segment = QUEUE_SEGMENT_BY_KIND[kind];
          return (
            <div
              key={kind}
            ...
            >
              ...
              <div className="mt-4 flex items-center justify-between border-t border-ink/5 pt-3 text-xs">
                ...
                {breached ? (
                  <Link
                    href={`/admin/queues/${segment}?filter=breached`}
                    title="Show SLA-breached items only"
                    className="inline-flex items-center gap-1 rounded-full bg-ember/10 px-2 py-0.5 font-bold uppercase tracking-wider text-ember transition-colors hover:bg-ember/20 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ember"
                  >
