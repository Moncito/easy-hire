...
export default async function JobDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="relative">
      <div
        className="pointer-events-none absolute inset-x-0 top-0 h-56 border-b border-navy/5 bg-[radial-gradient(ellipse_at_top_left,_rgba(30,58,95,0.08),_transparent_55%)]"
        aria-hidden="true"
      />
      ...
      <div className="relative mx-auto max-w-5xl px-6 py-10">
        <Link
          href="/jobs"
          className="mb-8 inline-flex cursor-pointer items-center gap-1.5 text-sm font-medium text-ink/50 transition hover:text-navy"
        >
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
          <div className="space-y-8 lg:col-span-2">
            ...
            <section className="border-t border-navy/8 pt-8">
              <h2 className="text-xs font-bold uppercase tracking-wider text-navy/55">About the role</h2>
              <p className="mt-3 whitespace-pre-wrap text-sm leading-relaxed text-ink/75">{job.description}</p>
            </section>

            {job.requirements && (
              <section className="border-t border-navy/8 pt-8">
                <h2 className="text-xs font-bold uppercase tracking-wider text-navy/55">Requirements</h2>
            ...
            {job.benefits && (
              <section className="border-t border-navy/8 pt-8">
                <h2 className="text-xs font-bold uppercase tracking-wider text-navy/55">Benefits</h2>
          ...
          <aside className="space-y-4">
            <div className="rounded-2xl border border-navy/8 bg-white p-6 shadow-[0_8px_30px_rgba(30,58,95,0.04)] lg:sticky lg:top-28">
              <div className="flex items-center gap-3">
                {job.company.logoUrl ? (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img src={job.company.logoUrl} alt="" className="h-14 w-14 rounded-xl object-cover" />
                ) : (
                  <div className="flex h-14 w-14 items-center justify-center rounded-xl bg-navy/8 font-display text-lg font-bold text-navy">
                    {initials}
                  </div>
                )}
                <div>
              ...
              {job.company.website && (
                <a
                  ...
                  className="mt-4 inline-flex cursor-pointer items-center gap-1.5 text-sm text-teal hover:underline"
                >
              ...
              <Link
                ...
                className="mt-3 flex cursor-pointer items-center gap-1.5 text-sm font-medium text-ink/55 hover:text-teal"
              >
                <Building2 className="h-4 w-4" aria-hidden="true" />
                View company profile
              </Link>

              <div className="mt-6 rounded-xl border border-marigold/20 bg-marigold/8 p-4">
                <p className="text-sm font-semibold text-ink">Ready to apply?</p>
                <p className="mt-1 text-xs leading-relaxed text-ink/50">
                  Submit your profile and resume. Employers review applications on their board.
                </p>
